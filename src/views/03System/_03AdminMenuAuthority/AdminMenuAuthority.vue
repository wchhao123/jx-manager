      ...
      <el-tree :data="menuList"
        ...
        <span class="custom-tree-node" slot-scope="{node, data}">
          ...
          <span>
            ...
            <el-button type="text" size="mini" @click.stop="_deleteMenu(data)"
                       v-show="isOwnDelete"
...
<script>
  ...
  import MenuAuthDetail from './AdminAuthDetail'
  ...
  export default {
    ...
    computed: {
      isOwnAdd() {
        return this.$store.getters.getBtnIsShowByName('btn_admin_menu_add')
      },
      isOwnDelete() {
        return this.$store.getters.getBtnIsShowByName('btn_admin_menu_delete')
      },
      isOwnUpdate() {
        return this.$store.getters.getBtnIsShowByName('btn_admin_menu_update')
      }
    },
    methods: {
      doQuery() {
      ...
      _deleteMenu(data) {
        this.$confirm(`确定要删除 < ${data.name} >菜单?`, '提示', {
